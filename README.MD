...
| fish selltonpc `range` | Option for `fish sellfishes` | Sell to Fishing Supplie. `range` is distance in meters for npc contact, 'ethical' is 6m. Note: npc contact exploit fixed on some regions, so max distance is 6m.|
